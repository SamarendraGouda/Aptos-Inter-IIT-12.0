...
TRADING_ACCOUNT_ADDRESS = ACCOUNT.account_address
...
def create_market(BASE_COIN: str, QUOTE_COIN: str, DEF_COIN: str, lot_size: int, tick_size: int, min_size: int) -> int:
    ...
    if market_id == None:
        ...
        calldata = register_market_base_coin_from_coinstore(ECONIA_ADDR,
                                                            TypeTag(
                                                                StructTag.from_str(BASE_COIN)),
                                                            TypeTag(
                                                                StructTag.from_str(QUOTE_COIN)),
                                                            TypeTag(
                                                                StructTag.from_str(DEF_COIN)),
                                                            lot_size,
                                                            tick_size,
                                                            min_size,
                                                            )
...
def deposit_into_market_account(market_id: int, amount: int, COIN_TYPE: str):
...
def get_market_account_balance(market_id: int, ACCOUNT) -> Tuple[int, int]:
    viewer = EconiaViewer(NODE_URL, ECONIA_ADDR)
...
def get_market_account_balance(market_id: int) -> Tuple[int, int]:
    viewer = EconiaViewer(NODE_URL, ECONIA_ADDR)
    market_account = get_market_account(
        viewer, ACCOUNT.account_address, market_id, 0)
...
def get_filled_orders(market_id: int) -> 'list[dict]':
    viewer = EconiaViewer(NODE_URL, ECONIA_ADDR)
...
def change_order_size(market_id: int, order_id: int, new_size: int, side: Side):
    econia_client = EconiaClient(NODE_URL, ECONIA_ADDR, ACCOUNT)
    call_data = change_order_size_user(
        ECONIA_ADDR, market_id, side, order_id, new_size)
    exec_txn(econia_client, call_data, f"Order size changed to {new_size}")


def cancel_order(market_id: int, order_id: int, side: Side):
    econia_client = EconiaClient(NODE_URL, ECONIA_ADDR, ACCOUNT)
    call_data = cancel_all_orders_user(ECONIA_ADDR, market_id, side)
    exec_txn(econia_client, call_data,
             f"All {side} orders cancelled for {ACCOUNT.address}")


def get_open_orders(market_id: int) -> 'list[dict]':
    viewer = EconiaViewer(NODE_URL, ECONIA_ADDR)
    ...
    if n_open_orders == 0:
        ...
        print(
            f"  * There are {n_open_orders} open orders for the queried account")
    return open_orders
