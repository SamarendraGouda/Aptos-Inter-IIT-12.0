#[test_only]
module Wallet_addr::Wallet_tests{
    use std::string::utf8;
    use std::signer;
    use std::unit_test;
    use std::vector;
    ...
    public entry fun test_create_storage_when_deposit_first_time() acquires Capabilities {
        ...
        Wallet::deposit<USDC>(&alice, 50u64);
        
        assert!(
          Wallet::depositedBalance<USDC>(alice_addr) == 50u64,
          0
        );
        assert!(
    ...
    public entry fun test_update_storage_when_deposit_next_time() acquires Capabilities {
        ...
        Wallet::deposit<USDC>(&alice, 20u64);
        
        assert!(
          Wallet::depositedBalance<USDC>(alice_addr) == 70u64,
          0
        );
        assert!(
    ...
    public entry fun test_withdraw() acquires Capabilities {
        ...
        Wallet::withdraw<USDC>(&alice, 20u64);
        
        assert!(
          Wallet::depositedBalance<USDC>(alice_addr) == 30u64,
          0
        );
        assert!(
